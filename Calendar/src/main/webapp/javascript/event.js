...
function setEventPanelStyle() {
	var left = window.innerWidth * .85714285714285716
			- parseInt(evint.panel.style.width) - 3;
...
function addCancelListener() {
	...
	while (i < eventPanel.buttonLabels.length) {
		...
		if (buttonLabel == "Delete" || buttonLabel == "Cancel"
				|| buttonLabel == "OK") {
...
document.getElementById("sc2750").addEventListener("click", deselectEvents,
		true);
